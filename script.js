  ...
  function printTo5(i) {
    ...
    if (i > 5) {
      ...
      steps.push({
        action: "RETURN",
        i,
        message: `Returning from printTo5(${i})`
      });
...
function getState() {
  ...
  for (let index = 0; index < currentStep; index++) {
    ...
    if (step.action === "RETURN") {
      if (stack.length && stack[stack.length - 1] === step.i) {
        stack.pop();
      }
    }
  }
...
function renderStack(stack) {
  ...
  stack.forEach((i, index) => {
    ...
    frame.textContent = `printTo5(i = ${i})`;

    const y = bottomY - index * (boxHeight + gap);
...
function renderCurrentStep() {
  ...
  if (currentStep >= steps.length) {
    ...
    message.textContent = "Program finished";
    details.innerHTML = `
      Stack is empty.<br>
  ...
  details.innerHTML = `
    ...
    <strong>Function:</strong> printTo5<br>
